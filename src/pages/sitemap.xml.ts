...
export const GET:APIRoute=({site})=>{
 const paths=[...legacyPages.map(p=>p.slug),'','servicios','sectores','casos-reales','blog','contacto',...pages.map(p=>p.slug),...sectors.map(p=>'sectores/'+p.slug),...cases.map(p=>'casos-reales/'+p.slug),...journal.map(p=>'blog/'+p.slug)];
